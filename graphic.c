...
/*void gruba_pozioma(int X1, int Y1,int szer){
	int Y11 = Y1;
	if(szer>0){
		for(int i = 0; i < 6; i++){
			GLCD_Line(X1, Y11, X1+szer-1, Y11);
			Y11++;
		}
	}
}*/
...
void gruba_pozioma(int X1, int Y,int szer) {
	if(szer>0){
		for(int i = 0; i < 6; i++){
			for(int j = 0; j < szer; j++){
				GLCD_SetPixel(X1+j, Y+i, 1);
			}
		}
	}
}
...
void gruba_pozioma_czysc(int X1, int Y,int szer){
	if(szer>0){
		for(int i = 0; i < 6; i++){
			for(int j = 0; j < szer; j++){
				GLCD_SetPixel(X1+j, Y+i, 0);
			}
		}
	}
}
